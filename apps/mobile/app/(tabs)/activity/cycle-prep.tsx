...
const CYCLE_CHECKLIST = ["Wear a helmet", "Choose a route"] as const;

export default function CyclePrepScreen() {
  ...
  const allChecked = CYCLE_CHECKLIST.every((_, i) => checked[i]);
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create({
        ...
        cardTitle: { fontSize: scaleW(16), fontWeight: "700", color: "#1A2E1E" },
        cardTitleChecked: { color: "#4F6F52", textDecorationLine: "line-through" },
        footer: {
          position: "absolute",
          bottom: 0,
          left: 0,
    ...
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTextWrap}>
          ...
          <ThemedText style={styles.headerSubtext}>Tick these off, then you&apos;re ready!</ThemedText>
        </View>
      ...
      >
        {CYCLE_CHECKLIST.map((title, index) => (
          <Pressable
            key={title}
          ...
          >
            <View style={[styles.checkbox, checked[index] && styles.checkboxChecked]}>
              {checked[index] && <MaterialIcons name="check" size={scaleW(16)} color="#FFF" />}
            </View>
            <ThemedText type="heading" style={[styles.cardTitle, checked[index] && styles.cardTitleChecked]}>
              {title}
            </ThemedText>
          </Pressable>
        ))}
      ...
      <View style={styles.footer} pointerEvents="box-none">
        <ThemedText style={styles.footerHint}>
          {teamId == null ? "Join a team." : allChecked ? "Awesome — let’s go!" : "Check both boxes to continue."}
        </ThemedText>
        <Pressable
          style={styles.startButton}
          disabled={!allChecked || teamId == null}
          onPress={() => router.push("/(tabs)/activity/cycle-map")}
        >
