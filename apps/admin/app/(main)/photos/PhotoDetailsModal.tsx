      ...
      >
        ...
        >
          <button
            ...
            className="relative flex min-h-0 w-full shrink-0 cursor-pointer items-center justify-center bg-stone-100 focus:outline-none focus:ring-2 focus:ring-huntly-sage focus:ring-inset sm:w-1/2 sm:border-r sm:border-stone-200"
          ...
          >
            <Image
              ...
              width={900}
              height={1200}
              className="max-h-[40dvh] w-auto object-contain sm:max-h-[90vh]"
            />
          </button>
                    ...
                    <DetailsSection title="Photo">
                      {data.photo && (
                        <dl className="space-y-1.5 text-sm">
                          <Row
                            label="Uploaded"
                            value={new Date(data.photo.uploaded_at).toLocaleString("en-GB", {
                              dateStyle: "medium",
                              timeStyle: "short",
                            })}
                          />
                          <Row
                            label="Status"
                            value={
                              STATUS_LABELS[data.photo.status] ?? String(data.photo.status)
                            }
                          />
                          {data.photo.status === STATUS_DENIED &&
                            data.photo.reason != null &&
                            data.photo.reason !== "" && (
                              <Row
                                label="Denial reason"
                                value={data.photo.reason}
