...
export function collectProximityMetrics(
  ...
  const water = minDistanceToMatching(point, classified, isWaterProps);
  const road = minDistanceToMatching(point, classified, isMajorRoadProps);
  const school = minDistanceToMatching(point, classified, isSchoolProps);
  const barrier = nearestBarrier(point, classified);
  return {
...
export function evaluateSafetyContext(
  ...
  const metrics = collectProximityMetrics(point, config, classified);
  ...
  const waterside =
    ...
    options.sourceType === "cycleway_walk" &&
      sourceProps?.towpath === "yes";

  // --- Water ---
  // Mapped public paths beside ponds/canals are valid standing places.
  // Hard-reject only when the point is inside water, or an area-grid cell
  // sits on the shore without an explicit path source.
  ...
  if (barrier.meters != null && barrier.type) {
    ...
    if (isGate && barrier.meters < config.gateBufferMeters) {
      // Public / unknown gate: do not auto-reject; flag + later confidence penalty.
  ...
  if (metrics.distanceToBboxEdgeMeters < config.bboxEdgeBufferMeters) {
    ...
    // Line candidates near edge: soft — flag only (confidence penalty later).
  }
