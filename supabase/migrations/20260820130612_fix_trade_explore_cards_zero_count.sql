-- Fix trade_explore_cards: the decrement UPDATE itself violated
-- explore_profile_cards_count_positive the instant count would hit 0,
-- before the function ever reached the "then delete" branch. Branch on
-- the already-locked v_current_count before mutating instead.
...
BEGIN
  ...
  -- Now mutate, still holding the row lock acquired above. Branch on the
  -- already-known count so we never attempt to write a count-0 row (the
  -- CHECK constraint forbids it) -- delete outright at exactly 5, otherwise
  -- decrement.
