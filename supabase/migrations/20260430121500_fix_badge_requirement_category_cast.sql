-- Fix legacy string requirement_category values (e.g. 'bird')
-- so badge progress RPC doesn't fail when casting to bigint.
...
BEGIN
  ...
  IF p_requirement_type = 'activities_by_category' THEN
    ...
    -- New format: category id as text (e.g. '3')
    IF v_trimmed_category ~ '^[0-9]+$' THEN
      v_category_id := v_trimmed_category::bigint;
    ELSE
      -- Legacy format: category slug/name (e.g. 'bird')
