...
import type {
  ...
  MultiPolygon,
  Polygon,
} from "npm:@types/geojson@7946.0.16";
import { LINE_ALTERNATIVE_OFFSETS_METERS, STEP3_BASELINE } from "./config.ts";
import { scoreConfidence } from "./confidence.ts";
import { mainEnvironment, scoreEnvironment } from "./environment.ts";
import {
  AREA_SOURCE_TYPES,
  LINE_SOURCE_TYPES,
...
function finalizeAccepted(
  ...
  const environmentProfile = scoreEnvironment(point, config, classified, c.sourceType);
  const priorityKey =
...
export function generateStops(
  ...
  for (const item of sources) {
    ...
    if (LINE_SOURCE_TYPES.has(sourceType)) {
      ...
      for (const candidate of candidates) {
        const result = tryLineAlternatives(candidate, item, config, classified);
      ...
      sourceCandidatesGenerated += candidates.length;
      for (const candidate of candidates) {
        const result = tryAreaAlternatives(candidate, item, config, classified);
        if ("accepted" in result) {
          alternativePositionsTested += result.accepted.alternativeIndex + 1;
          survivors.push(result.accepted);
        } else {
  ...
  for (const c of spaced.accepted) {
    const stop = finalizeAccepted(c, config, classified, sourceProps);
